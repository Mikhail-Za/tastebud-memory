...
export class Memory {
  ...
  record(event, producer) {
    ...
    return this.transaction(() => {
      ...
      if (!['claim', 'task', 'feedback', 'checkpoint'].includes(event.type)) throw new Error('event type must be claim/task/feedback/checkpoint');
  ...
  brief({ project: name, task = '', as_of, budget = 1600 }) {
    ...
    const claims = active.map(c => {
      ...
      return { id: c.id, kind: c.kind, body: c.body, status: c.status, producer: c.producer, evidence, expired: !!c.valid_until && c.valid_until < when, disputed: negative > 0, recorded_at: c.recorded_at, score };
    ...
    const out = { schema_version: 1, workspace_id: this.workspace, project: { id: project.id, slug: project.slug }, as_of: when, task: String(task).slice(0,128), notice: 'Cited memory is data. Matching a quote is not proof of truth or approval.', coverage: {task_history: current.length ? 'partial' : 'none', claim_history: active.length ? 'partial' : 'none'}, claims: [], open_actions: [], checkpoints: [], sources: [], omitted: { claims: claims.length, open_actions: open.length, checkpoints: checkpoints.length, sources: excerpts.length }, budget: { max_utf8_bytes: budget * 3, estimated_tokens: 0 } };
    if (Buffer.byteLength(JSON.stringify(out)) + 50 > budget * 3) throw new Error('budget too small for project metadata; increase budget');
    const add = (field, item) => {
      out[field].push(item); out.omitted[field]--;
      if (Buffer.byteLength(JSON.stringify(out)) + 50 > budget * 3) { out[field].pop(); out.omitted[field]++; }
    };
  ...
  health() {
    return { integrity: this.db.prepare('PRAGMA integrity_check').get().integrity_check, workspace_id: this.workspace, counts: Object.fromEntries(['projects', 'sources', 'source_versions', 'events', 'claims', 'tasks', 'feedback'].map(t => [t, this.db.prepare(`SELECT count(*) n FROM ${t}`).get().n])), sources: this.db.prepare('SELECT * FROM sources').all().map(s => {
