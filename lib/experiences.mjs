...
const ref = (memory, id, row, type) => {
  ...
  if (prior.seq >= row.seq || prior.occurred_at > row.occurred_at) throw new Error(`${id} must be a strictly earlier event`);
...
export function validateExperience(memory, row, payload) {
  ...
  if (payload.supersedes != null) {
    ...
    const all = rows(memory).filter(r => r.seq < row.seq && r.type === 'experience');
...
export function validateApplication(memory, row, payload) {
  ...
  const all = rows(memory).filter(r => r.seq < row.seq), experiences = all.filter(r => r.type === 'experience');
  ...
  if (!current) {
    ...
    if (current.project_id !== row.project_id || current.payload.experience_id !== payload.experience_id) throw new Error('application identity, project and source experience are immutable');
...
const diagnosticEvidence = (memory, evidence) => evidence.map(e => {
  ...
  return { ...e, path: source?.path, captured_source_changed: source?.current_hash !== e.hash, filesystem_status: !source ? 'missing-record' : !existsSync(path) ? source.archived ? 'archived-in-store' : 'missing-file' : hash(readFileSync(path)) !== source.current_hash ? 'changed' : 'current', archived: !!source?.archived, diagnostics_as_of: 'current' };
});

function applicationSummary(memory, all, experience, reader) {
  ...
  const priority = values => values.map(expose).sort((a,b) => Number(b.outcome === 'failed')-Number(a.outcome === 'failed') || b.seq-a.seq);
  ...
  return { counts, historical_counts, target_projects: targets.slice(0, 50), target_projects_omitted: Math.max(0, targets.length-50), histories: histories.slice(0, 20), omitted: Math.max(0, histories.length-20), history: history.slice(0, 20), history_omitted: Math.max(0, history.length-20) };
}
...
export function candidates(memory, args = {}) {
  ...
  const suggestions = exps.filter(e => lineageCurrent(exps, e) && !e.payload.retired && sourceVisible(e, project)).map(e => {
    ...
    const applications = applicationSummary(memory, all, e, project);
